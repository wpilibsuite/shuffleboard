...
public final class StreamDiscoverer implements AutoCloseable {
  ...
  public StreamDiscoverer(NetworkTable publisherTable, String cameraName) {
    streamsSub =
        publisherTable
            ...
            .getStringArrayTopic(STREAMS_KEY)
            .subscribe(emptyStringArray);
    streamsListener =
        publisherTable
            .getInstance()
            .addListener(
  ...
  /**
   * Removes leading camera protocols from an array of stream URLs. These URLs are usually in the
   * format {@code mjpg:http://...}, {@code ip:http://...}. This method will remove the leading
   * {@code mjpg}. This will also replace a trailing <code>/?action=stream</code> with <code>
   * /stream.mjpg?</code> due to a bug in cscore not handling URL parameters correctly.
   *
