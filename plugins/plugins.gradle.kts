...
subprojects {
    afterEvaluate {
        /** The name of the plugin. This is, by convention, entirely lower-case. */
        val pluginName: String = project.name
        /** The plugin name as a proper noun, with the first character capitalized */
        val proper = Character.toUpperCase(pluginName[0]) + pluginName.substring(1)
        ...
        val versionClassFileName = "src/main/java/edu/wpi/first/shuffleboard/plugin/$pluginName/${proper}PluginVersion.java"
        ...
        val checkForVersionUpdate = task("checkPluginVersionUpdated") {
            ...
            doLast {
                ...
                if (changedFiles.isNotEmpty()) {
                    val versionFileChanged = !fileExistsOnMaster("plugins/$pluginName/$versionClassFileName") || changedFiles.any { file ->
                        file.endsWith(versionClassFileName)
                    }
                    if (!versionFileChanged) {
                        throw VersionNotUpdatedException("Version file was not updated for plugin '$pluginName'")
                    }
                    val pluginFile = project.java.sourceSets["main"].allJava.toList()
                            .first { file -> file.nameWithoutExtension.equals("${pluginName}PluginVersion", true) }
                    ...
                    if (masterVersion.contains("public static final String VERSION = \"$version\"")) {
                        throw VersionNotUpdatedException("Plugin version was not updated for plugin '$pluginName'")
                    }
                }
            }
        }
        ...
        val generateVersionClass = task("generateVersionClass") {
            if (version == null || version == "" || version == "unspecified") {
                throw GradleException("No version specified for plugin '$pluginName' (given: $version)")
            }
            description = "Generates a source file containing the plugin version string"
            val versionClassContent = """
                package edu.wpi.first.shuffleboard.plugin.$pluginName;

                /**
                 * Autogenerated version class containing the version of the $pluginName plugin.
                 *
                ...
                public final class ${proper}PluginVersion {
                  ...
                  /**
                   * The version of the $pluginName plugin.
                   */
