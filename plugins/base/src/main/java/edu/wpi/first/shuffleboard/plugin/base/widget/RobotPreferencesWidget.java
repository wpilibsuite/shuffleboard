...
public class RobotPreferencesWidget extends SimpleAnnotatedWidget<RobotPreferencesData> {
  ...
  // If we just used the data map in the property sheet, a new item (and editor) would be created
  // for each change
  // For example, typing a new character in a text field would remove that field and replace it with
  // a new one
  // This approach makes it so that editors don't appear to lose focus when users are interacting
  // with them
  private final ObservableMap<String, ObjectProperty<Object>> wrapperProperties =
          ...
          updated.forEach(
              (key, value) -> {
                if (DataSourceUtils.isNotMetadata(key)) {
                  wrapperProperties
                      .computeIfAbsent(key, k -> generateWrapper(k, value))
                      .setValue(value);
                }
              });
        });

    wrapperProperties.addListener(
        (MapChangeListener<String, ObjectProperty<? super Object>>)
            change -> {
              if (change.wasAdded()) {
                propertySheet
                    .getItems()
                    .add(
                        new ExtendedPropertySheet.PropertyItem<>(
  ...
  public List<Group> getSettings() {
    return ImmutableList.of(
        Group.of(
            "Miscellaneous",
            Setting.of(
                "Show search box", propertySheet.searchBoxVisibleProperty(), Boolean.class)));
  }
