...
@Description(
    name = "Encoder",
    dataTypes = {EncoderType.class, QuadratureEncoderType.class})
...
public class EncoderWidget extends SimpleAnnotatedWidget<EncoderData> {
  ...
  private void initialize() {
    distanceField
        .textProperty()
        .bind(EasyBind.monadic(dataProperty()).map(EncoderData::getDistance).map(Object::toString));
    speedField
        .textProperty()
        .bind(EasyBind.monadic(dataProperty()).map(EncoderData::getSpeed).map(Object::toString));
  }
