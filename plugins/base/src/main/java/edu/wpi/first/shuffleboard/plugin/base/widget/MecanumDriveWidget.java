...
public final class MecanumDriveWidget extends AbstractDriveWidget<MecanumDriveData> {
  ...
  @FXML private VBox leftControls;
  @FXML private StackPane driveView;
  @FXML private VBox rightControls;
  @FXML private Pane vectorPane;

  private final BooleanProperty showForceVectors =
      new SimpleBooleanProperty(this, "showForceVectors", true);
  ...
  private MonadicBinding<DataSource<SpeedControllerData>>
      frontLeftMotorSource; // NOPMD could be local variable
  private MonadicBinding<DataSource<SpeedControllerData>>
      frontRightMotorSource; // NOPMD could be local variable
  private MonadicBinding<DataSource<SpeedControllerData>>
      rearLeftMotorSource; // NOPMD could be local variable
  private MonadicBinding<DataSource<SpeedControllerData>>
      rearRightMotorSource; // NOPMD could be local variable

  /**
   * Marks the positions of mecanum wheels on a drive base. This is used to determine the
   * orientation to draw a wheel. Note that {@link #FRONT_LEFT} and {@link #REAR_RIGHT} are
   * functionally identical, as well as {@link #FRONT_RIGHT} and {@link #REAR_LEFT}.
   */
  ...
  public List<Group> getSettings() {
    return ImmutableList.of(
        Group.of("Visuals", Setting.of("Show velocity vectors", showForceVectors, Boolean.class)));
  }
    ...
    if (direction.getMagnitude() > 0.01) {
      ...
      directionArrow =
          CurvedArrow.createStraight(
              direction.getMagnitude() * vectorRadius, -direction.getAngle(), 0, 8);
    }
   ...
   */
  private static Shape generateMecanumDriveBase(
      double wheelWidth, double wheelHeight, double frameWidth, double frameHeight) {
  ...
  private static Shape mecanumWheel(double width, double height, MecanumWheelPos pos) {
    ...
    for (int i = 0; i <= numLines + 1; i++) {
      ...
      line.setFill(null);
      Shape intersect =
          Shape.intersect(boundingBox, line); // Make sure the line doesn't go outside the wheel
