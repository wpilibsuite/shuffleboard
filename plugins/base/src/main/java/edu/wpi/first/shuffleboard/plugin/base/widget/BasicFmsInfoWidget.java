...
public class BasicFmsInfoWidget extends SimpleAnnotatedWidget<FmsInfo> {
  ...
  @FXML private Label robotStateLabel;
  @FXML private Label fmsConnectionLabel;
  @FXML private Label driverStationConnectionLabel;
  @FXML private Pane emergencyStopPane;
  ...
  private void initialize() {
    ...
    fmsConnectionLabel
        ...
        .bind(
            dataOrDefault.map(
                info ->
                    "FMS "
                        + (info.getFmsControlData().isFmsAttached()
                            ? "connected"
                            : "disconnected")));
    driverStationConnectionLabel
        .textProperty()
        .bind(
            dataOrDefault.map(
                info ->
                    "DriverStation "
                        + (info.getFmsControlData().isDsAttached()
                            ? "connected"
                            : "disconnected")));
    fmsConnectionLabel
        .graphicProperty()
    ...
    robotStateLabel
        ...
        .bind(
            dataOrDefault.map(
                info -> "Robot state: " + info.getFmsControlData().getControlState().name()));

    emergencyStopPane
        .managedProperty()
        .bind(dataOrDefault.map(FmsInfo::getFmsControlData).map(ControlWord::isEmergencyStopped));
