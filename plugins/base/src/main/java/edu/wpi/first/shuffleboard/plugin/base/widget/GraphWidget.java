...
@Description(
    name = "Graph",
    dataTypes = {Number.class, double[].class})
...
public class GraphWidget extends AbstractWidget implements AnnotatedWidget {
  ...
  /**
   * Keep track of all graph widgets so they update at the same time. It's jarring to see a bunch of
   * graphs all updating at different times
   */
  ...
  public static class Updater implements AutoCloseable {
    ...
    private final IntegerProperty graphUpdateRate =
        new SimpleIntegerProperty(this, "graphUpdateRate", 10);
    private final ScheduledExecutorService executorService =
        ThreadUtils.newDaemonScheduledExecutorService();
    private volatile ScheduledFuture<?> currentFuture;

    private final ChangeListener<Number> updateCreator =
        (observable, oldValue, newValue) -> {
          ...
          currentFuture =
              executorService.scheduleAtFixedRate(
                  this::updateAll, 500, amount, TimeUnit.MILLISECONDS);
        };
  ...
  private void initialize() {
    ...
    yAxisAutoRanging.addListener(
        (__, was, useAutoRanging) -> {
          if (useAutoRanging) {
    ...
    chart
        ...
        .bind(Bindings.createBooleanBinding(() -> sources.size() > 1, sources));
    sources.addListener(
        (ListChangeListener<DataSource>)
            c -> {
              while (c.next()) {
                if (c.wasAdded()) {
                  c.getAddedSubList()
                          ...
                          source -> {
                            if (source.getDataType() == NumberType.Instance) {
                              ...
                              if (source.isConnected()) {
                                numberChangeListener.changed(
                                    source.dataProperty(), null, (Number) source.getData());
                              }
                            } else if (source.getDataType() == NumberArrayType.Instance) {
                              source.dataProperty().addListener(numberArrayChangeListener);
                              if (source.isConnected()) {
                                numberArrayChangeListener.changed(
                                    source.dataProperty(), null, (double[]) source.getData());
                              }
                            } else {
                              throw new IncompatibleSourceException(
                                  getDataTypes(), source.getDataType());
                            }
                          });
        ...
        new StringConverter<>() {
          ...
          private String toString(int seconds, boolean negative) {
            return String.format(
                "%s%d:%02d", negative ? "-" : "", (seconds / 60) % 60, seconds % 60);
          }
    ...
    ActionList.registerSupplier(
        root, () -> ActionList.withName(getTitle()).addAction("Clear", this::clear));
    ...
    ActionList.registerSupplier(
        ...
        () ->
            ActionList.withName(getTitle())
                .addAction(
                    "Toggle X-Axis Autoscroll",
                    () -> this.xAxisAutoScrolling.set(!this.xAxisAutoScrolling.get())));

    // Add this widget to the list only after everything is initialized to prevent occasional null
    // pointers when
    ...
    chart
        ...
        .forEach(
            s -> {
              ...
              doubleDataSet.lock().writeLockGuard(doubleDataSet::clearData);
            });
  }
  ...
  private void updateFromNumberSource(DataSource<? extends Number> source) {
    ...
    // The update HAS TO run on the FX thread, otherwise we run the risk of
  ...
  private void updateSeries(DoubleDataSet data, long now, double nextValue) {
    ...
    data.lock()
        .writeLockGuard(
            () -> {
              // So getValues() does not return an array of all the elements, but instead returns
              // the
              ...
              if (data.getDataCount(DataSet.DIM_Y) > 1
                  && yValues[yValues.length - 1] != nextValue) {
    ...
    boolean dataVisible =
        Optional.ofNullable(visibleSeries.get(data)).map(Property::getValue).orElseThrow();
              ...
              .readLockGuard(
                  () -> {
                    ...
                    return OptionalDouble.of(
                        xValues[doubleDataSet.getDataCount(DataSet.DIM_X) - 1]);
                  });
    ...
    return ImmutableList.of(
        Group.of(
            ...
            Setting.of(
                ...
                "Automatically scroll the x-axis",
                xAxisAutoScrolling,
                Boolean.class)),
        // Note: users can set the lower bound to be greater than the upper bound, resulting in an
        // upside-down graph
        Group.of(
            "Y-axis",
            Setting.of(
            ...
            Setting.of(
                ...
                yAxisMinBound,
                Double.class),
            Setting.of("Unit", "The unit displayed on the y-axis", yAxisUnit, String.class)),
        Group.of(
            "Visible data",
            visibleSeries.values().stream()
