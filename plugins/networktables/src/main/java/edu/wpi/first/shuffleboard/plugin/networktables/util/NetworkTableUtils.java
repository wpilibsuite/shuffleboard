...
public final class NetworkTableUtils {
  ...
  public static DataType dataTypeForEntry(String key) {
    ...
    if (rootTable.containsKey(normalKey)) {
      return DataTypes.getDefault()
          .forJavaType(rootTable.getEntry(normalKey).getValue().getValue().getClass())
          .get();
    }
    if (rootTable.containsSubTable(normalKey)) {
      ...
      if (type == null) {
        ...
        return DataTypes.getDefault().forName(type).orElse(DataTypes.Map);
      }
    }
    return null;
  }
   ...
   * This should be used when changing from server mode to client mode, or changing server address
