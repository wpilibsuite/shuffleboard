...
public class SingleKeyNetworkTableSource<T> extends NetworkTableSource<T> {
  ...
  /**
   * Flag marking whether or not an update from the table is the first update the source gets. This
   * works around the issue caused when the initial value received from NetworkTables is the same as
   * the default value provided by the data type, which results in the source never being marked
   * active.
   */
  private volatile boolean initialUpdate = true;

  /**
   * Creates a single-key network table source backed by the value in the given table associated
   * with the given key.
   *
   * @param table the table backing the source
   * @param key the key associated with the data
   * @param dataType the allowable data type. A value that is not an instance of this type is
   *     considered "null" and will make the source inactive
   */
