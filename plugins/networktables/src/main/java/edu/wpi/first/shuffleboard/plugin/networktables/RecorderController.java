...
/**
 * Controls the {@link Recorder Shuffleboard data recorder} in response to changes in NetworkTables.
 * Changes to the file name format entry will only take effect when the next recording starts, to
 * avoid splitting data across multiple recording files.
 */
public final class RecorderController {
  ...
  public static final String DEFAULT_FILE_NAME_FORMAT_KEY =
      DEFAULT_RECORDING_ROOT_TABLE + "/FileNameFormat";
   ...
   *
   * @param ntInstance the NetworkTable instance to connect to
   * @param startStopKey the key for the entry used to control the state of the recorder. The entry
   *     must contain boolean values for any action to be taken
   * @param fileNameFormatKey the key for the entry used to control the file names of recording
   *     files. The entry must contain a String value for it to be used
  ...
  public RecorderController(
    ...
    fileNameFormatSub =
        ntInstance
            .getStringTopic(fileNameFormatKey)
            .subscribe(Recorder.DEFAULT_RECORDING_FILE_NAME_FORMAT);
    this.recorder = recorder;
    this.markerGenerator = new MarkerGenerator(ntInstance, recorder);
  }

  /**
   * Starts this controller. If the control entry is set, the recorder will immediately start if it
  ...
  /**
   * Updates the state of the data recorder in response to a change to the networktable control
   * entry. If the entry does not contain a boolean value, or if Shuffleboard is in data playback
   * mode, this method will do nothing.
   *
