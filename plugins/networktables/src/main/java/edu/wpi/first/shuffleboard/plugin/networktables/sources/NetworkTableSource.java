        ...
        inst.addListener(
            ...
            event -> {
              ...
              if (isSingular() && !name.equals(fullTableKey)) {
                // Since NetworkTableInstance.addEntryListener() will fire on anything that starts
                // with the key,
                // a singular source will be notified for an unrelated entry.
                // For example, a singular source for the entry "/S" will also be fired for any
                // changing entry that
                // starts with "/S", such as "/SmartDashboard/<anything>" or "/SomeUnrelatedEntry".
                // This check prevents the source from being erroneously updated for an unrelated
                // entry.
                return;
              }
              if (isConnected()) {
  ...
  public static DataSource<?> forKey(String fullTableKey) {
    ...
    if (NetworkTableUtils.rootTable.containsKey(key)) {
      ...
      return sources.computeIfAbsent(
          uri,
          __ ->
              new SingleKeyNetworkTableSource<>(
    ...
    if (NetworkTableUtils.rootTable.containsSubTable(key) || key.isEmpty()) {
      ...
      return sources.computeIfAbsent(
          uri,
          __ ->
              new CompositeNetworkTableSource(
                  key, (ComplexDataType) NetworkTableUtils.dataTypeForEntry(key)));
    }
    return DataSource.none();
  }

  /**
   * Gets the existing data source for the given key, or {@link Optional#empty()} is no source has
   * been created for that key.
   */
