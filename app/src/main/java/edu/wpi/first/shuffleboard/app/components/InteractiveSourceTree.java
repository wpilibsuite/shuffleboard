...
/**
 * An interactive source tree that allows user gestures. This encapsulates the logic used to drag
 * sources from a tree into a widget pane, and handles context menus and updating when the available
 * sources change.
 */
public final class InteractiveSourceTree extends SourceTreeTable<SourceEntry, Object> {
  ...
  private SourceEntry
      selectedEntry; // NOPMD could be final - false positive. PMD doesn't seem to know what a
  // lambda is
   ...
   * @param addComponentToActiveTab a callback used to add a new component to the active tab. Used
   *     by the "Show as" context menu
   * @param createTabForSource a callback used to create and bring focus to a new tab to
   *     autopopulate with data in a complex data source
   */
  public InteractiveSourceTree(
    ...
    setOnContextMenuRequested(
        e -> {
          ...
          if (source.getDataType().isComplex()) {
            menu.getItems()
                .add(FxUtils.menuItem("Create tab", __ -> createTabForSource.accept(entry)));
  ...
  private MenuItem createShowAsMenuItem(String componentName, DataSource<?> source) {
    return FxUtils.menuItem(
        "Show as: " + componentName,
        __ -> {
          Components.getDefault()
              .createComponent(componentName, source)
