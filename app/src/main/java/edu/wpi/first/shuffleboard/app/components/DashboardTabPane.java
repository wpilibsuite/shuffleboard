...
public class DashboardTabPane extends TabPane {
  ...
  private final Map<TabStructure, Map<TabModel, ProcedurallyDefinedTab>> pluginTabs =
      new WeakHashMap<>();

  private final StructureChangeListener structureChangeListener =
      tabs -> {
        FxUtils.runOnFxThread(
            () -> {
              Map<TabModel, ProcedurallyDefinedTab> realTabs =
                  pluginTabs.computeIfAbsent(tabs, __ -> new WeakHashMap<>());
              for (TabModel model : tabs.getTabs().values()) {
                var tab =
                    realTabs.computeIfAbsent(
                        model, m -> new ProcedurallyDefinedTab(m, PropertyParsers.getDefault()));
  ...
  /**
   * Adds a new tab at the end of the tab list. The default name is "Tab <i>n</i>", where <i>n</i>
   * is the number of dashboard tabs in the pane.
   *
  ...
  /**
   * Selects the tab with the given index. If the index is negative or larger than the highest
   * index, no tab is selected.
   *
   * @param tabIndex the index of the tab to select
   */
  public void selectTab(int tabIndex) {
    if (tabIndex >= 0
        && tabIndex < getTabs().size()
        && getTabs().get(tabIndex) instanceof DashboardTab) {
  ...
  /**
   * Add a component to the active tab pane. Should only be done by the result of specific user
   * interaction.
   */
  ...
  public void selectWidgets(Predicate<Widget> selector) {
    getTabs().stream()
        .map(optionalCast(DashboardTab.class))
        .forEach(
            tab ->
                tab.map(DashboardTab::getWidgetPane)
