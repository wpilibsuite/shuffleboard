...
public final class LeftDrawerController {
  ...
  @FXML private Accordion sourcesAccordion;
  @FXML private WidgetGallery widgetGallery;
  @FXML private Pane handle;
  @FXML private Labeled expandContractButton;
  ...
  private void setup(Plugin plugin) {
    FxUtils.runOnFxThread(
        () -> {
          plugin
              ...
              .forEach(
                  sourceType -> {
                    ...
                    tree.getFilterableRoot()
                        .predicateProperty()
                        .bind(
                            EasyBind.monadic(searchField.textProperty())
                                .map(
                                    text -> {
                                      if (text.isEmpty()) {
                                        ...
                                        return (parent, entry) ->
                                            StringUtils.containsIgnoreCase(
                                                    entry.getViewName(), text)
                                                || StringUtils.containsIgnoreCase(
                                                    entry.getName(), text)
                                                || (entry.getValue() != null
                                                    && StringUtils.containsIgnoreCase(
                                                        StringUtils.deepToString(entry.getValue()),
                                                        text));
                                      }
                                    }));
                    ...
                    Glyph searchIcon =
                        GlyphFontRegistry.font("FontAwesome").create(FontAwesome.Glyph.SEARCH);
                    ...
                    FXCollections.sort(
                        sourcesAccordion.getPanes(), Comparator.comparing(TitledPane::getText));
          ...
          widgetGallery.setWidgets(
              Components.getDefault().allWidgets().collect(Collectors.toList()));
        });
  }

  /**
   * Removes all traces from a plugin from the left drawer. Source trees will be removed and all
   * widgets defined by the plugin will be removed from the gallery.
   */
