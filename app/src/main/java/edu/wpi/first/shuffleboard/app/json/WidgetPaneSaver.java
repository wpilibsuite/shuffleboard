...
public class WidgetPaneSaver implements ElementTypeAdapter<WidgetPane> {
  ...
  @Override
  public WidgetPane deserialize(JsonElement json, JsonDeserializationContext context)
      throws JsonParseException {
    JsonObject object = json.getAsJsonObject();
    WidgetPane pane = new WidgetPane();
    pane.setShowGrid(
        Optional.ofNullable(object.get("showGrid")).map(JsonElement::getAsBoolean).orElse(true));
    ...
    for (Map.Entry<String, JsonElement> tileLocation : tiles.entrySet()) {
      ...
      GridPoint coords =
          new GridPoint(Integer.parseInt(coordPart[0]), Integer.parseInt(coordPart[1]));
