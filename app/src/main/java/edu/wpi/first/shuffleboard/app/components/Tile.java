...
public class Tile<T extends Component> extends BorderPane {
  ...
  private final MonadicBinding<Pane> contentView =
      EasyBind.monadic(content).map(Component::getView); // NOPMD
  ...
  private final PropertyBinding<String> contentTitle = // NOPMD local variable
      EasyBind.monadic(content).selectProperty(Component::titleProperty);
   ...
   * however, the tile will not change size. The size must be set separately with {@link
   * #setSize(TileSize)}.
   */
  ...
  /**
   * Sets the size of this tile. This does not directly change the size of the tile; it is up to the
   * parent pane to actually resize this tile.
   */
