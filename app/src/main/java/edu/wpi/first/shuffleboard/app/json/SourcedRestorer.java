...
public class SourcedRestorer {
  ...
  /**
   * Adds a {@link DestroyedSource} for each supported data type of a sourced object. The
   * application will handle restoring the sources when possible.
   *
   ...
   * @param errorHandler a function to call when a source could not be restored, or when a restored
   *     source could not be added
   */
  public void restoreSourcesFor(
      Sourced sourced,
    ...
    List<DestroyedSource> toRestore =
        sourced.getSources().stream()
    ...
    for (DestroyedSource source : toRestore) {
      try {
        ...
        // Remove all destroyed sources with the same ID; they were only present to allow us to
        // restore the source
        // with the correct data type.  Since restoring this source was successful, the correct data
        // type is known and
        // the remaining destroyed sources are no longer necessary
        sourced
            .getSources()
            .removeIf(s -> s instanceof DestroyedSource && s.getId().equals(source.getId()));
      } catch (Throwable e) {
