...
/**
 * Helper class for shrinking and moving tiles around to attempt to keep them all in the bounds of
 * their widget pane.
 */
public class TileMover {
  ...
  /**
   * Collapses or moves the given tile in the given direction by up to <code>count</code> rows or
   * columns. Tiles will be moved to fill empty spaces, then the rightmost/bottommost tiles will be
   * shrunk until they reach their minimum size, which is content-dependent.
   *
  ...
  /**
   * Creates a {@link Runnable} that will move or shrink the given tile, as well as moving or
   * shrinking any tiles in the way of moving it. If the tile cannot be shrunk or moved, returns an
   * empty Optional.
   *
   ...
   */
  private Optional<Runnable> collapseTile(
      Tile tile,
    ...
    TileSize minSize =
        pane.round(
            tile.getContent().getView().getMinWidth(), tile.getContent().getView().getMinHeight());
    ...
    if (!pane.isOverlapping(targetLayout, n -> n == tile)
        && !targetLayout.origin.equals(layout.origin)) { // NOPMD
      ...
      List<Optional<Runnable>> runs =
          IntStream.range(lower, upper)
              .mapToObj(
                  i ->
                      left
                          ? pane.tileAt(targetLayout.origin.col, i)
                          : pane.tileAt(i, targetLayout.origin.row))
              .flatMap(TypeUtils.optionalStream()) // guaranteed to be at least one tile
              .distinct() // need to make sure we have no repeats, or n-row tiles will get moved n
              // times
              .filter(t -> tile != t)
