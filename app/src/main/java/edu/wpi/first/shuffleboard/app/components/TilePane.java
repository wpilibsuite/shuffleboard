...
public class TilePane extends GridPane {
  ...
  public double tileSizeToWidth(int tileWidth) {
    checkArgument(
        tileWidth >= 1, "The tile size must be a positive integer (was " + tileWidth + ")");
  ...
  public double tileSizeToHeight(int tileHeight) {
    checkArgument(
        tileHeight >= 1, "The tile size must be a positive integer (was " + tileHeight + ")");
  ...
  /**
   * Finds the first point where a tile with the given size can be added, or {@code null} if no such
   * point exists.
   *
  ...
  public boolean isOpen(int col, int row, int tileWidth, int tileHeight, Predicate<Node> ignore) {
    if (col < 0
        || col + tileWidth > getNumColumns()
        || row < 0
        || row + tileHeight > getNumRows()) {
      return false;
    }
  ...
  /**
   * Identical to {@link #isOverlapping(int, int, int, int, Predicate)} but takes a tile layout that
   * contains the row, column, width, and height data.
   *
  ...
  public boolean isOverlapping(TileLayout layout, Predicate<Node> ignore) {
    return isOverlapping(
        ...
        layout.size.getHeight(),
        ignore);
  }

  /**
   * Checks if a tile with the given width and height would overlap a pre-existing tile at the point
   * {@code (col, row)}, ignoring some nodes when calculating collisions. Note that this method does
   * <i>not</i> perform bounds checking; use {@link #isOpen(int, int, int, int, Predicate) isOpen}
   * to check if a widget can be placed at that point.
   *
   * @param col the column index of the point to check
   * @param row the row index of the point to check
   * @param tileWidth the width of the tile
  ...
  public boolean isOverlapping(
      int col, int row, int tileWidth, int tileHeight, Predicate<Node> ignore) {
    for (Node child : getChildren()) {
      if (ignore.test(child)) {
        continue;
      }
      // All "real" children have a column index, row index, and column and row spans
      // Other children (like the grid lines) don't have these properties and will throw null
      // pointers
      ...
      if (GridPane.getColumnIndex(child) != null) {
        ...
        if (x + width > col && y + height > row && x - tileWidth < col && y - tileHeight < row) {
