...
@SuppressWarnings(
    "PMD.MoreThanOneLogger") // there's only one logger used, the others are for setting up file
// logging
public class Shuffleboard extends Application {
  ...
  public void init() throws AlreadyLockedException, IOException, InterruptedException {
    ...
    // Search for and load themes from the custom theme directory before loading application
    // preferences
    // This avoids an issue with attempting to load a theme at startup that hasn't yet been
    // registered
    ...
    notifyPreloader(
        new ShuffleboardPreloader.StateNotification("Loading NetworkTables plugin", 0.25));
    PluginLoader.getDefault().load(new NetworkTablesPlugin());
    notifyPreloader(
        new ShuffleboardPreloader.StateNotification("Loading CameraServer plugin", 0.375));
    ...
    notifyPreloader(
        new ShuffleboardPreloader.StateNotification("Initializing user interface", 0.875));
    FXMLLoader loader = new FXMLLoader(MainWindowController.class.getResource("MainWindow.fxml"));
    mainPane = loader.load();
    long fxmlLoadTime = fxmlLoadTimer.elapsed(TimeUnit.MILLISECONDS);
    mainWindowController = loader.getController();
    logger.log(
        fxmlLoadTime >= 500 ? Level.WARNING : Level.INFO,
        "Took " + fxmlLoadTime + "ms to load the main FXML");

    notifyPreloader(new ShuffleboardPreloader.StateNotification("Starting up", 1));
    Thread.sleep(
        20); // small wait to let the status be visible - the preloader doesn't get notifications
    // for a bit
  }
  ...
  public void start(Stage primaryStage) {
    ...
    // Must be called in start() because init() is run on the main thread, not the FX application
    // thread
    ...
    logger.log(
        startupTime > 5000 ? Level.WARNING : Level.INFO,
        "Took " + startupTime + "ms to start Shuffleboard");
  }
  ...
  /**
   * Logs an uncaught exception on a thread. This is in a method instead of directly in a lambda to
   * make the log a bit cleaner ({@code edu.wpi.first.shuffleboard.app.Shuffleboard
   * uncaughtException} vs {@code edu.wpi.first.shuffleboard.app.Shuffleboard start$lambda$2$}).
   *
  ...
  /**
   * Gets the time at which the application JAR was built, or the instant this was first called if
   * shuffleboard is not running from a JAR.
   */
  ...
  public static String getVersion() {
    // Try to get the version from the shuffleboard class. This will return null when running from
    // source (eg using
    // gradle run or similar), so in that case we fall back to getting the version from an API
    // class, which will always
  ...
  /**
   * Gets the location from which shuffleboard is running. If running from a JAR, this will be the
   * location of the JAR; otherwise, it will likely be the root build directory of the `app`
   * project.
   */
  public static String getRunningLocation() {
    try {
      return new File(
              Shuffleboard.class.getProtectionDomain().getCodeSource().getLocation().toURI())
          .getAbsolutePath();
