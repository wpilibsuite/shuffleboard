...
/**
 * This is a class that is designed to be used as a Tab graphic, it adds title editing and an
 * onDragOver event.
 */
public class TabHandle extends StackPane {

  private static final int DRAG_FOCUS_DELAY = 500;
  private final ScheduledExecutorService delayedDragService =
      ThreadUtils.newDaemonScheduledExecutorService();
  private final HandledTab tab;

  private final Property<Node> tabHeaderContainer =
      new SimpleObjectProperty<>(this, "tabHeaderContainer");
  private final EventHandler<Event> fireFromTabContainer =
      event -> {
  ...
  /**
   * Creates a TabHandle for a specific Tab. This TabHandle should be retained by the tab, or else
   * specific event handlers might be garbage collected.
   */
  public TabHandle(HandledTab tab) {
    this.tab = tab;

    tabHeaderContainer.addListener(
        (obs, oldContainer, newContainer) -> {
    ...
    tabHeaderContainer.bind(
        EasyBind.select(parentProperty())
            .select(Node::parentProperty)
            .selectObject(Node::parentProperty));
  ...
  private void startDelayedDrag(DragEvent dragEvent) {
    Future<?> task =
        delayedDragService.schedule(
            () -> FxUtils.runOnFxThread(tab::onDragOver), DRAG_FOCUS_DELAY, TimeUnit.MILLISECONDS);
