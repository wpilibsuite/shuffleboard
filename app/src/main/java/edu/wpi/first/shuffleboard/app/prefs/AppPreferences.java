...
/**
 * Contains the user preferences for the app. These preferences are user-specific and are not
 * contained in save files.
 */
public final class AppPreferences {
  ...
  private final Category settings =
      Category.of(
          "App Settings",
          Group.of("Theme", Setting.of("Theme", "The theme to display Shuffleboard with", theme)),
          Group.of(
              "Tab Settings",
              Setting.of(
                  "Default tile size",
                  "The tile size of new tabs (existing tabs are unaffected)",
                  defaultTileSize),
              Setting.of(
                  "Confirm tab close",
                  "Requests confirmation when closing a tab",
                  confirmTabClose)),
          Group.of(
              "Startup",
              Setting.of(
                  "Load last save file",
                  "Load the most recent save file at startup",
                  autoLoadLastSaveFile)),
          Group.of(
              "Miscellaneous",
              Setting.of("Confirm exit", "Request confirmation before exiting", confirmExit)));

  @VisibleForTesting static AppPreferences instance = new AppPreferences();
  ...
  public AppPreferences() {
    ...
    autoLoadLastSaveFile.addListener(
        __ -> PreferencesUtils.save(autoLoadLastSaveFile, preferences));
