...
public class PluginPaneController {
  ...
  @FXML private TableView<Plugin> pluginTable;
  @FXML private TableColumn<Plugin, String> nameColumn;
  @FXML private TableColumn<Plugin, Boolean> loadedColumn;
  ...
  private void initialize() {
    ...
    versionColumn.setCellValueFactory(
        data -> new ReadOnlyStringWrapper(data.getValue().getVersion().toString()));
        ...
        data -> {
          ...
          prop.addListener(
              (__, was, is) -> {
                ...
                // Force update the checkboxes to make sure they're disabled when their plugin can't
                // be loaded
    ...
    MonadicBinding<String> desc =
        EasyBind.monadic(pluginTable.selectionModelProperty())
    ...
    files.forEach(
        f -> {
          try {
            ...
            if (!f.getParentFile().toPath().equals(Storage.getPluginPath())
                && !jarUris.contains(jarUri)) {
              jarUris.add(jarUri);
            }
