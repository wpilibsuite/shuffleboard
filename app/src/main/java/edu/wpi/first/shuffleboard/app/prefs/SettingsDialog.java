...
/**
 * A dialog for editing {@link edu.wpi.first.shuffleboard.api.prefs.Setting settings}. Each {@link
 * Category} gets an entry in a list on the left-hand side of the dialog. Selecting a category will
 * display its property sheet in the main right-hand view. The first category in the list will be
 * selected by default.
 */
public final class SettingsDialog extends Dialog<Boolean> {
  ...
  public SettingsDialog(Collection<Category> categories) {
    ...
    Rectangle2D visualBounds =
        Screen.getPrimary().getVisualBounds(); // TODO use the screen the app is displayed on
    getDialogPane()
        .setPrefSize(
