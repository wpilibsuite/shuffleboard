...
public final class JsonBuilder {
  ...
  private static final ImmutableList<Class<? extends ElementTypeAdapter<?>>> builtinAdapters =
      ImmutableList.of(
          ColorSaver.class,
    ...
    try {
      for (Class<?> c : builtinAdapters) {
        builder.registerTypeHierarchyAdapter(
            c.getAnnotation(AnnotatedTypeAdapter.class).forType(), c.newInstance());
      }
  ...
  public static Gson forSaveFile() {
    return withTypeAdapter().setPrettyPrinting().create();
  }
}
