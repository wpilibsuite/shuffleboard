...
public abstract class AbstractWidget implements Widget {
  ...
  private final Property<FontAwesome.Glyph> glyph =
      new SimpleObjectProperty<>(this, "glyph", FontAwesome.Glyph.CUBE);
  private final BooleanProperty showGlyph = new SimpleBooleanProperty(this, "showGlyph", false);

  private final ChangeListener<Boolean> connectionListener =
      (__, was, is) -> {
        if (is) {
  ...
  /**
   * Only set the title when the sources change if it hasn't been set externally (eg by a user or an
   * enclosing layout).
   */
        ...
        (ListChangeListener<DataSource>)
            c -> {
              ...
              while (c.next()) {
                if (c.wasAdded()) {
                  c.getAddedSubList()
                      .forEach(s -> s.connectedProperty().addListener(connectionListener));
                } else if (c.wasRemoved()) {
                  c.getRemoved()
                      .forEach(s -> s.connectedProperty().removeListener(connectionListener));
                }
              }
            });
  }
