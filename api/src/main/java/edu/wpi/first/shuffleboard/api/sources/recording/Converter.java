...
/**
 * Handles converting of recording files to a file. These are useful for making it easier to analyze
 * recorded data with third-party tools like Excel or other spreadsheet applications.
 *
...
public interface Converter {
  ...
  Comparator<RecordingEntry> markersFirst =
      (first, second) -> {
  ...
  /**
   * The file extension for the output files, including the dot. For example, ".csv" or ".txt" for
  ...
  /**
   * Gets the settings for this converter. Defaults to an empty list; implementations may override
   * this behavior to provide custom settings.
   */
  ...
  /**
   * Flattens recording entries into a single map of timestamp-to-data. Note that each timestamp is
   * expected to have no more than ONE event marker mapped to it.
   *
   * @param recording the recording to flatten
   * @param window the time window within which temporally-close data should be considered to have
   *     the same timestamp. A value of zero will result in only entries for that timestamp being
   *     mapped to it; higher values loosens this restriction to accommodate potential variances in
   *     timestamps.
  ...
  static Map<Long, List<RecordingEntry>> flatten(
      Recording recording, Predicate<TimestampedData> filter, long window) {
    if (window < 0) {
    ...
    // All the timestamped data and event markers, sorted by timestamp in ascending order (oldest
    // data first)
    final List<RecordingEntry> data =
        Stream.concat(recording.getData().stream(), recording.getMarkers().stream())
    ...
    for (int i = 0; i < data.size(); ) {
      ...
      int j = i + 1;
      // Collate data within a certain delta time to the same collection, since there may be some
      // time jitter
