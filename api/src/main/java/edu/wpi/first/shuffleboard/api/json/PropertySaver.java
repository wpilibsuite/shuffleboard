...
/**
 * Saves property fields annotated with {@link SaveThisProperty @SaveThisProperty} and properties of
 * fields annotated with {@link SavePropertyFrom @SavePropertyFrom}. Annotated fields from the
 * entire superclass hierarchy are saved, not just fields from the object being serialized.
 */
public final class PropertySaver {
   ...
   *
   * <ol>
   *   <li>Properties exported via {@link Component#getSettings()}
   *   <li>Properties from fields annotated with {@link SaveThisProperty} in the component's class
   *       hierarchy
   *   <li>Properties from fields annotated with {@link SavePropertyFrom} in the component's class
   *       hierarchy
   * </ol>
   *
  ...
  public void saveAllProperties(
      Component object, JsonSerializationContext context, JsonObject jsonObject) {
    ...
    for (Group group : object.getSettings()) {
      for (Setting<?> setting : group.getSettings()) {
        ...
        if (!savedProperties.contains(property)) {
          Class<?> type =
              setting.getType() == null ? setting.getProperty().getClass() : setting.getType();
          serializeProperty(
              context, jsonObject, property, type, group.getName() + "/" + setting.getName());
        }
      }
    }
   ...
   * @param jsonObject the JSON object to serialize into
   */
  public void saveAnnotatedFields(
      Object object, JsonSerializationContext context, JsonObject jsonObject) {
    getPropertyFields(object.getClass())
        .forEach(
            f -> {
              ...
              serializeProperty(
                  context, jsonObject, property, property.getValue().getClass(), name);
            });
  }

  /**
   * Saves nested properties from fields annotated ith {@link SavePropertyFrom @SavePropertyFrom} in
   ...
   * @param jsonObject the JSON object to serialize into
   */
  public void saveNestedProperties(
      Object object, JsonSerializationContext context, JsonObject jsonObject) {
              ...
              for (SavePropertyFrom annotation : annotations) {
                ...
                try {
                  ...
                  throw new RuntimeException(
                      "Could not get value of property '" + propName + "' of " + f.getType(), e);
                }
              }
            });
  }
  ...
  public void readAllProperties(
      Component object, JsonDeserializationContext context, JsonObject jsonObject) {
    ...
    for (Group group : object.getSettings()) {
      for (Setting setting : group.getSettings()) {
        ...
        Object deserialized =
            context.deserialize(jsonObject.get(group.getName() + "/" + setting.getName()), type);
   ...
   */
  public void readAnnotatedFields(
      Object object, JsonDeserializationContext context, JsonObject jsonObject) {
    getPropertyFields(object.getClass())
        .forEach(
            f -> {
              Property property = ReflectionUtils.getUnchecked(object, f);
              String name = getSavedName(property, f.getAnnotation(SaveThisProperty.class));
              Object deserialize =
                  context.deserialize(jsonObject.get(name), property.getValue().getClass());
  ...
  /**
   * Deserialized nested properties from fields annotated ith {@link
   * SavePropertyFrom @SavePropertyFrom} in the object's class hierarchy.
   *
   * @param object the object containing the properties to deserialize
   * @param context the deserialization context
   * @param jsonObject the JSON object to deserialize
   */
  public void readNestedProperties(
      Object object, JsonDeserializationContext context, JsonObject jsonObject) {
    getNestedPropertyFields(object.getClass())
        .forEach(
            f -> {
              ...
              for (SavePropertyFrom annotation : annotations) {
                ...
                try {
                  ...
                  Object deserialize =
                      context.deserialize(jsonObject.get(savedName), set.getParameterTypes()[0]);
                  // Can be null if not set
                  // Instead of throwing IllegalArgumentException, just let the default value be
                  // used
                  if (deserialize != null) {
   ...
   * {@code get<name>} e.g. {@code getFoo()}. The method must be public. Boolean properties can also
   * be in the form {@code is<name>}, e.g. {@code isFoo()}.
   *
   * @param clazz the class containing the property
   * @param propertyName the name of the property
   * @return the getter method for the property
   * @throws IllegalArgumentException if there is not exactly ONE method matching {@code public Foo
   *     get<property>()} or {@code public boolean is<property>()}
   */
  public static Method getGetter(Class<?> clazz, String propertyName) {
    ...
    String isName =
        "is" + base; // NOPMD linguistics antipattern - this is the name of a boolean "isX" method
    List<Method> possibleGetters =
    ...
    if (possibleGetters.isEmpty()) {
      throw new IllegalArgumentException(
          "No getters for property '" + propertyName + "' in " + clazz);
    } else if (possibleGetters.size() > 1) {
      throw new IllegalArgumentException(
          "Multiple getters for property " + propertyName + ": " + possibleGetters);
    }
  ...
  /**
   * Gets the setter method in the given class for a property with the given name, in the form
   * {@code set<name>} e.g. {@code setFoo()}. The method must be public, and cannot have any
   * overrides (for example {@code public void setFoo(Foo)} and {@code public void setFoo(Bar)}
   * would cause problems).
   *
   * @param clazz the class containing the property
   * @param propertyName the name of the property
   * @return the setter method for the property
   * @throws IllegalArgumentException if there is not exactly ONE method matching {@code public void
   *     set<property>(<type>)}
   */
  private static Method getSetter(Class<?> clazz, String propertyName) {
    ...
    if (possibleSetters.isEmpty()) {
      throw new IllegalArgumentException(
          "No setter for property " + propertyName + " in " + clazz.getName());
    } else if (possibleSetters.size() > 1) {
      throw new IllegalArgumentException(
          "Too many setters for property "
              + propertyName
              + ": "
              + possibleSetters.stream().map(Method::getName).collect(Collectors.joining(", ")));
    }
    return possibleSetters.get(0);
  }

  /**
   * Gets a stream of the fields in the object's class hierarchy annotated with {@link
   * SaveThisProperty @SaveThisProperty}.
   */
  public static Stream<Field> getPropertyFields(Class<?> clazz) {
    if (clazz == null) {
      ...
      return Stream.concat(
          getPropertyFields(clazz.getSuperclass()),
  ...
  private static Stream<Field> getNestedPropertyFields(Class<?> clazz) {
    if (clazz == null) {
      ...
      return Stream.concat(
          getNestedPropertyFields(clazz.getSuperclass()),
          Arrays.stream(clazz.getDeclaredFields())
  ...
  /**
   * Gets the saved name a property annotated with {@code @SaveThisProperty}. If the name specified
   * in the annotation is an empty string (the default value), then the property's name will be
   * used. Otherwise, the saved name will be the one specified in the annotation.
   *
   ...
   * @throws IllegalArgumentException if the property has no name (either null or empty string) and
   *     the annotation does not specify the property name
   */
  private static String getSavedName(Property<?> property, SaveThisProperty annotation) {
    if (annotation.name().isEmpty()) {
      if (property.getName() == null || property.getName().isEmpty()) {
        throw new IllegalArgumentException(
            "The property has no name, and no name was specified in the annotation");
      }
  ...
  private static String getPropertyName(SavePropertyFrom annotation) {
    if (annotation.savedName().isEmpty()) {
      return annotation
          .propertyName(); // Don't need to check this - it's done before this method is called
    } else {
