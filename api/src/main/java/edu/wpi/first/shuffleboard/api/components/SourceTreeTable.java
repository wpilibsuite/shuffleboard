...
/**
 * A tree table view or displaying hierarchical sources. Note: the root item <strong>must</strong>
 * be a {@link FilterableTreeItem}. This allows source trees to be searchable. Not setting the root
 * item to a {@code FilterableTreeItem} will result in exceptions being thrown when items in the
 * tree table are added, removed, or updated.
 *
...
public class SourceTreeTable<S extends SourceEntry, V> extends TreeTableView<S> {
  ...
  private final ObjectProperty<SourceType> sourceType =
      new SimpleObjectProperty<>(this, "sourceType", null);
