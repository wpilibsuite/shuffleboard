...
/**
 * A utility class for generating curved arrows. The generated arrows follow a <i>circular</i>,
 * rather than <i>elliptical</i>, path. The heads of the arrows are always the same isosceles
 * triangle with the length of the base equalling the height of the triangle; this works out to be
 * (approximately) a 63-63-54 triangle.
 */
public final class CurvedArrow {
  ...
  public static Shape create(
      double startAngle, double radius, double length, double xOffset, double headSize) {
    if (radius < 0) {
  ...
  /**
   * Creates a new curved arrow. This is equivalent to calling {@link #create create(startAngle,
   * radius, radius * sweepAngle, xOffset, headSize)}.
   *
   * @param startAngle the starting angle of the arc, in radians
   ...
   * @throws IllegalArgumentException if {@code radius} or {@code headSize} are negative
   */
  public static Shape createPolar(
      double startAngle, double radius, double sweepAngle, double xOffset, double headSize) {
  ...
  private static Triangle straightHead(
      double startAngle, double size, double xOffset, double bodyLength) {
   ...
   */
  private static Triangle curvedHead(
      double startAngle, double size, double arcRadius, double xOffset, double arcLength) {
    ...
    // If the length is negative, the tip is closer to the start of the arc and therefore needs to
    // be
    ...
    // (arcLength + size) / arcRadius - startAngle
    // These are combined into a single calculating by multiplying `size` by the sign of the arc
    // length
    ...
    Point2D tipPoint =
        new Point2D(tipRadius * Math.cos(angleToTip) + xOffset, tipRadius * Math.sin(angleToTip));
