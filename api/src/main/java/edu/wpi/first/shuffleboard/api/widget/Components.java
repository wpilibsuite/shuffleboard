...
public class Components extends Registry<ComponentType> {
  ...
  public void register(ComponentType type) {
    if (components.containsKey(type.getName())) {
      throw new IllegalArgumentException(
          "Component class " + type.getClass().getName() + " is already registered");
    }
  ...
  /**
   * Validates a component class. An exception is thrown if the class does not have a {@link
   * Description @Description} annotation, or if its name is empty.
   *
   * @throws InvalidWidgetException if the component class is not valid
   */
  public static <T extends Component> void validateAnnotatedComponentClass(
  ...
  public void unregister(ComponentType type) {
    ...
    List<DataType> defaultComponentsToRemove =
        defaultComponents.entrySet().stream()
   ...
   * @return an optional containing the created view, or an empty optional if no widget could be
   *     created
   */
  ...
  /**
   * Tries to create a widget from a known widget with the given name. If successful, the widgets
   * data sources will be set to the given sources. If any of these sources are incompatible, an
   * {@link IncompatibleSourceException} will be thrown.
   *
   * @param name the name of the widget to create
   * @param sources the data sources for the widget to use
   * @return an optional containing the created widget, or an empty optional if no widget could be
   *     created
   * @throws IncompatibleSourceException if the widget for the given name is incompatible with any
  ...
  public Optional<Widget> createWidget(String name) {
    Optional<Widget> widget =
        typeFor(name).map(ComponentType::get).flatMap(TypeUtils.optionalCast(Widget.class));
  ...
  public Optional<? extends Component> createComponent(String name) {
    ...
    Optional<Widget> widget =
        typeFor(name).filter(WidgetType.class::isInstance).flatMap(__ -> createWidget(name));
    widget.ifPresent(this::setId);
    if (widget.isPresent()) {
      return widget;
    } else {
  ...
  /**
   * Creates a new component with the given name. If the component takes a source (ie implements
   * {@link Sourced}), its source will be set to the one provided.
   *
   * @param name the name of the component to create
   * @param source the source for the created component to use, if the component accepts one
   * @return an optional containing the created component, or empty if no component with the given
   *     name is registered
   */
  ...
  public Optional<Component> getByUuid(UUID uuid) {
    return ImmutableMap.copyOf(allActiveComponents).entrySet().stream()
   ...
   *
   * <p><strong>Do not keep references to elements in this list.</strong> It prevents garbage
   * collection of widget instances.
   */
  ...
  /**
   * Gets the names of all the possible components that can display the given type, sorted
   * alphabetically. A component can be created for these with {@link #createWidget(String,
   * DataSource) createWidget}.
   *
   * @param type the type of data to get possible widgets for.
   * @return an alphabetically sorted list containing the names of all known widgets that can
   *     display data of the given type
   */
  public List<String> componentNamesForType(DataType type) {
    return getComponentsForType(type).stream()
  ...
  /**
   * Gets the name of the default component for the given data type, or {@link Optional#empty()} if
   * there is no default component for that type.
   */
  ...
  /**
   * Gets the name of a component that can handle data of the given type. If a default component has
   * been set for that type, the name of the default component is returned; otherwise, the name of
   * the first component returned by {@link #componentNamesForType(DataType)} is used.
   */
