...
/**
 * A widget is a UI element that displays data from {@link DataSource data sources} and has the
 * ability to modify that data. It is up to the widget implementation to modify the backing data;
 * some widgets (for example, a graph widget) may be read-only. Simple widgets can display singular
 * data points (eg some text, a number, a boolean value, etc); composite widgets display complex
 * data like subsystems or sensors.
 *
