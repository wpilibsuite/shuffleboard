...
/**
 * A type of source that provides a view of a subset of the data of another source, as well as
 * propagating data changes to the original source. This is especially helpful for widgets that
 * delegate some behaviour to another, embedded widget (typically used when a data type is a
 * superset of another; e.g. in FRC, PIDCommand data is a superset of both PIDController data and
 * Command data).
 */
public final class SubSource<T> extends AbstractDataSource<T> {
   ...
   * @param toBackingData a conversion function to use to convert the data of this source to data
   *     usable by the backing source
  ...
  public <U> SubSource(
    ...
    PropertyUtils.bindBidirectionalWithConverter(
        this.dataProperty(), source.dataProperty(), toBackingData, extractData);
