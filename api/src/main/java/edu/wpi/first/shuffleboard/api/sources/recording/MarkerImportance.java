...
public enum MarkerImportance {
  ...
  // Maintainer note: this enum is mirrored in WPILib. Adding or removing entries, or changing the
  // name of any entry,
  // requires a corresponding change to WPILib.

  /**
   * The lowest importance level. This can be used for marking the most minor events that are nice
   * to know if something goes wrong, but otherwise has no use.
   */
  TRIVIAL(0),
  /** Marks an event with low importance. */
  LOW(1),
  /**
   * Marks an event with no special connotations, such as the start of a match or match period, or
  ...
  HIGH(3),
  /**
   * Marks a critically important event such as a component failure, power loss, software deadlock,
   * or timeout.
   */
  CRITICAL(4);
