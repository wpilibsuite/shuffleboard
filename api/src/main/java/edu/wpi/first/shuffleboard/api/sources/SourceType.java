...
/**
 * Represents the <i>type</i> of a data source and where the data for that source comes from. There
 * may be arbitrarily many types of data sources for a single source type. For example, a
 * theoretical "file system" type may have one implementation of {@link DataSource} for files, and
 * another implementation for directories.
 */
public abstract class SourceType {
   ...
   * @param protocol the protocol string for source URIs using this source type. For example,
   *     'network_table://"
   * @param sourceSupplier a function to use to create data sources of this type for a given name
   *     (<i>not</i> URI)
   */
  public SourceType(
      String name,
   ...
   * {@link edu.wpi.first.shuffleboard.api.data.types.UnknownType Unknown} type. If no source is
   * known by the given URI, returns an {@link edu.wpi.first.shuffleboard.api.data.types.NoneType
   * None} type.
   *
  ...
  /**
   * Removes the protocol prefix from a string. Has no effect if the given text does not start with
   * this types protocol.
   *
  ...
  /**
   * Given a URI-like string with a protocol and a pseudo-path, return a source for the current
   * SourceType This function will throw an error if it doesn't support the protocol that's passed.
   *
   * <p>The interpretation of paths may vary depending on any given SourceType.
   */
  ...
  /**
   * Reads a data point and passes it to all appropriate sources of this type.The default behavior
   * is to do nothing; recordable subclasses <i> must </i> override this method.
   */
  ...
  /**
   * Disconnects sources of this type from the backing source interface. Sources may still be
   * modified by users, but changes made when in this state should not affect the source interface.
   */
  ...
  /**
   * Creates a root source entry. The entry will not be used to create a source, but to represent
   * the root node in the source tree view in the application window.
   */
  ...
  /**
   * Creates a source entry corresponding to the given URI. The default implementation throws an
   * exception; custom source types should override this behavior.
   *
