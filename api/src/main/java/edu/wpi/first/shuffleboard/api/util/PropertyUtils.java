...
public final class PropertyUtils {
   ...
   * target. The converters will not be called if a value changes to {@code null}; instead, the
  ...
  public static <T, U> void bindWithConverter(
      Property<T> firstProperty, Property<U> secondProperty, Function<U, T> u2tConverter) {
   ...
   */
  public static <K, V, T extends V> void bindToMapBidirectionally(
      Property<T> property, ObservableMap<K, V> map, K key, Function<V, T> v2t) {
   ...
   */
  public static <T> ObservableList<T> combineLists(
      ObservableList<T> first, ObservableList<T> second) {
        ...
        c -> {
          while (c.next()) {
            if (c.wasAdded()) {
              ...
              // Otherwise all instances of an element would be removed, which is not the desired
              // behavior
