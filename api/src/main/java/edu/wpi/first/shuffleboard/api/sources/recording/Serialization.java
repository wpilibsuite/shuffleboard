...
public final class Serialization {
  ...
  /**
   * A magic number that is always the first entry in a recording file. This helps check (but does
   * not guarantee) that a loaded file is a valid recording file.
   */
  public static final int MAGIC_NUMBER = 0xBEEF_FACE;

  /**
   * The current serialization format version. This number is incremented every time the recording
   * format changes in a way that makes it incompatible with previous versions.
   */
  ...
  private static final class Offsets {
    ...
    /**
     * The offset to the <code>int</code> value holding the position of the markers array in the
     * file. The byte at this position will be the first byte in the markers array, and is preceded
     * by the final byte of the constant pool.
     */
    public static final int MARKER_POSITION_OFFSET = NUMBER_DATA_POINTS_OFFSET + SIZE_OF_INT;

    /**
     * The offset to the <code>int</code> value holding the position of the data array in the file.
     * The byte at this position will be the first byte in the data array, and is preceded by the
     * final byte in the markers array.
     */
  ...
  public static void saveRecording(Recording recording, Path file) throws IOException {
    ...
    // Work on a copy, since the recording can have new data added to it while we're in the middle
    // of saving
    var snapshot = recording.takeSnapshotAndClear();
    final var dataCopy =
        snapshot.getData().stream().sorted().collect(ImmutableList.toImmutableList());
    ...
    put(
        header,
        toByteArray(segments.stream().mapToInt(b -> b.length).sum()),
        Offsets.MARKER_POSITION_OFFSET);
    ...
    for (TimestampedData data : dataCopy) {
      ...
      final byte[] dataType =
          toByteArray((short) constantPool.indexOf(data.getDataType().getName()));
    ...
    byte[] all = segments.stream().reduce(new byte[0], Bytes::concat);
  ...
  /**
   * Updates a saved recording file with the contents of the given recording. Note: the recording
   * should <i>not</i> contain any data that has already been saved to disk, or it will be saved
   * again.
   *
    ...
    try (RandomAccessFile raf = new RandomAccessFile(file.toFile(), "rw")) {
      ...
      if (magic != MAGIC_NUMBER) {
        throw new IOException(
            String.format(
                "Wrong magic number in the header. Expected 0x%08X, but was 0x%08X",
                MAGIC_NUMBER, magic));
      }
      ...
      for (int i = 0; i < constantPoolSize; i++) {
        ...
        String constantPoolEntry =
            new String(Arrays.copyOfRange(buffer, 0, len), StandardCharsets.UTF_8);
        pos += len;
        constantPoolLength += len;
        constantPoolEntries.add(constantPoolEntry);
      }

      Stream<String> sourceIdStream =
          dataCopy.stream().map(TimestampedData::getSourceId).distinct();
      Stream<String> dataTypeStream =
          dataCopy.stream().map(t -> t.getDataType().getName()).distinct();
      Stream<String> constantPoolStream = Stream.concat(sourceIdStream, dataTypeStream);
      byte[] newConstantPoolEntries =
          constantPoolStream
              .filter(id -> !constantPoolEntries.contains(id))
              .peek(constantPoolEntries::add)
              .map(Serialization::toByteArray)
              .reduce(new byte[0], Bytes::concat);
      byte[] newMarkerEntries =
          markers.stream()
              .map(
                  marker -> {
          ...
          dataCopy.stream()
              .map(
                  data -> {
                    ...
                    final byte[] timestamp = toByteArray(data.getTimestamp());
                    // use int16 instead of int32 -- 32,767 sources should be enough
                    final byte[] sourceIdIndex =
                        toByteArray((short) constantPoolEntries.indexOf(data.getSourceId()));
                    final byte[] dataType =
                        toByteArray((short) constantPoolEntries.indexOf(type.getName()));
                    final byte[] dataBytes = encode(value, type);
      ...
      if (newConstantPoolEntries.length > 0) {
        ...
        // Update the constant pool
        int constantPoolEnd =
            Offsets.CONSTANT_POOL_HEADER_OFFSET + SIZE_OF_INT + constantPoolLength;
  ...
  public static Recording loadRecording(Path file) throws IOException {
    ...
    if (version != VERSION) {
      throw new IOException(
          "Cannot load recording with format version "
              + version
    ...
    for (int i = 0; i < numMarkers; i++) {
      ...
      Marker marker =
          new Marker(name, description, MarkerImportance.forId(importanceId), timestamp);
  ...
  /**
   * Gets the size of a string array if it were encoded as a byte array with {@link
   * #toByteArray(String[])}.
   */
   ...
   *
   * <ul>
   *   <li>The {@link #MAGIC_NUMBER magic number}, to help confirm data integrity
   *   <li>The {@link #VERSION version number}, to avoid attempting to load incompatible recording
   *       files
   *   <li>The number of data points (signed 32-bit int)
   *   <li>The names of all the recorded sources, used for caching
   * </ul>
   */
  ...
  public static List<String> getAllSourceNames(List<TimestampedData> data) {
    return data.stream().map(TimestampedData::getSourceId).distinct().collect(Collectors.toList());
  }
  ...
  /**
   * Encodes a string as a big-endian byte array. The resulting array encodes the number bytes that
   * encode the string in a 4-byte int, followed by the encoded character bytes.
   */
  ...
  /**
   * Encodes a string array as a big-endian byte array. These can be read with {@link
  ...
  /**
   * Creates a new array with the same contents as {@code raw} in the range {@code (start, end]}.
   * Note: the two arrays are <i>distinct</i>; modifying one will <i>not</i> modify the other.
   *
  ...
  /**
   * Puts {@code src} into {@code dst} at the given position. This is a shortcut for {@code
  ...
  public static int readInt(byte[] array, int pos) {
    if (array.length < pos + SIZE_OF_INT) {
      throw new IllegalArgumentException(
          "Not enough bytes to read from. Starting position = "
              + pos
              + ", array length = "
              + array.length);
    }
