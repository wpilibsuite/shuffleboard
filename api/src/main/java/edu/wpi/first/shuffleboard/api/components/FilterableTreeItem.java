...
public class FilterableTreeItem<T extends SourceEntry> extends TreeItem<T> {
  ...
  private static final Comparator<TreeItem<? extends SourceEntry>> comparator =
      SourceTreeTable.branchesFirst.thenComparing(SourceTreeTable.alphabetical);
  ...
  private final ObjectProperty<TreeItemPredicate<T>> predicate =
      new SimpleObjectProperty<>(TreeItemPredicate.always());
  ...
  public FilterableTreeItem(T value) {
    ...
    filteredList
        .predicateProperty()
        .bind(EasyBind.monadic(predicate).map(this::createListFilter).orElse(always));
  ...
  /**
   * Recursively sorts all children of this item. Children are sorted by branches first, then
   * alphanumerically.
   */
