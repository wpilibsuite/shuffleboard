...
/**
 * Registry of data types in shuffleboard. This class also provides data types for various
 * "wildcard" types, as well as types for primitive and arrays of primitive data. These types are
 * always registered and may not be unregistered.
 */
public class DataTypes extends Registry<DataType> {
  ...
  /**
   * Represents the type of "null" or non-present data. Differs from {@link UnknownType} in that
   * <i>no</i> data is present for this type, while data <i>is</i> present but of an unknown type
   * for the latter.
   */
  public static final DataType None = NoneType.Instance;

  /**
   * Represents the type of <i>all</i> data; a widget that can accept this data type can accept data
   * of <i>any</i> type.
   */
  public static final DataType All = AllType.Instance;

  /**
   * Represents an "unknown" data type; that is, data is present, but the type could not be
   * determined.
   */
  ...
  /**
   * The type corresponding to an array of numeric data (<code>double[]</code>). Note that number
   * arrays <i>must</i> be implemented as <code>double[]</code> in order to be represented by this
   * type.
   */
  ...
  private static final ImmutableCollection<DataType<?>> defaultTypes =
      ImmutableSet.of(
          // catchall
          None,
          All,
  ...
  /**
   * Creates a new data type registry. The registry will initially contain the default data types,
   * none of which may be unregistered.
   */
  public DataTypes() {
  ...
  public void unregister(DataType dataType) {
    ...
    if (defaultTypes.contains(dataType)) {
      throw new IllegalArgumentException(
          "A default data type cannot be unregistered: '" + dataType + "'");
    }
    ...
    return (Optional)
        typeCache.computeIfAbsent(
            ...
            __ -> {
              ...
              Optional<DataType> naive =
                  dataTypes.values().stream().filter(t -> type.equals(t.getJavaClass())).findAny();
              if (naive.isPresent()) {
                return naive;
              } else {
                ...
                return dataTypes.values().stream()
                    ...
                    .sorted(
                        (t1, t2) ->
                            classComparator
                                .reversed()
                                .compare(t1.getJavaClass(), t2.getJavaClass()))
                    .findFirst();
              }
            });
  }
  ...
  /**
   * Creates a comparator object that compares classes in order of closest to the target class, in
   * terms of class hierarchy. For example, take a class hierarchy of
   *
   * <pre><code>
   *   A extends Object
   *   B extends A
   ...
   *
   * <p><b>Scenario 1</b> <br>
   * {@code closestTo(Object.class)} would be sorted as {@code A, B, [C, D]}. The order of C, D is
   * not deterministic and depends on the ordering of the source collection, hence the brackets.
   *
   * <p><b>Scenario 2</b> <br>
   * {@code closestTo(C.class)} would be sorted as {@code C, B, A, D}. {@code D} is at the very end
   * because it is not part of the class hierarchy of {@code C}; neither one subclasses the other.
   *
   * <p><b>Scenario 3</b> <br>
   * {@code closestTo(B.class)} would be sorted as {@code B, [A, C], D}. {@code A} and {@code C} are
   * equidistant from {@code B}, so their ordering would depend on the order of the source
   * collection.
   *
   * <p><b>This method does <i>not</i> support comparison of interfaces</b>
   *
  ...
  /**
   * Calculates the distance between two classes in the class hierarchy. This does <i>not</i>
   * support interfaces. If neither class subclasses the other, or either class object represents an
   * interface, this method will return {@link Integer#MAX_VALUE}.
   *
  ...
  public <D extends DataType> Optional<D> forType(Class<D> clazz) {
    return (Optional<D>) dataTypes.values().stream().filter(d -> d.getClass() == clazz).findFirst();
  }
