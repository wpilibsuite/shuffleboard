...
/**
 * A component that uses a bar to display a number. The bar can originate from any point along its
 * range, making this a good way to display numbers that can be in a range (eg -1 to 1, centered at
 * 0).
 *
 * <p>Note that this extends {@code RangeSlider} to take advantage of the code that controlsfx has
 * already written. This component is <i>not</i> controllable, and exists only to be used as a
 * read-only view.
 */
public class LinearIndicator extends RangeSlider {
  ...
  /**
   * Sets the low and high values as necessary based on changes to the current value or center
   * properties.
   */
