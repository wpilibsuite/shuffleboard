...
public class WidgetPaneController {
  ...
  private void dropSource(DataSource<?> source, GridPoint point) {
    ...
    pane.widgetForSource(source).ifPresent(node -> pane.moveNode(node, point));
  }
        ...
        .forEach(
            name -> {
              ...
              if (name.equals(widget.getName())) {
                ...
                changeItem.setOnAction(
                    __ -> {
                      pane.removeWidget(tile);
                      Widgets.createWidget(name, widget.getSource()).ifPresent(pane::addWidget);
                    });
              }
